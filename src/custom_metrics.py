...
class JaccardIndex():
    ...
    def update_state_frame(self, y_true, y_pred):
        '''Input in coordinates...'''
        self.frame_compute_jaccard(y_true, y_pred)
    ...
    def frame_compute_jaccard(self, truth, pred):
        """
        define false positive: localisation without ground truth for distance > 100nm
        define false negative: ground truth without localisation for distance > 100nm
        :param ground_truth:
        ...
        for k in range(pred.shape[0]):
            found = False
            min_dis = 100
            for l in range(truth.shape[0]):
                dis = np.linalg.norm(pred[k] - truth[l])
                if dis<min_dis:
                    if k not in t_pred and l not in t_truth:
                        min_dis = dis
                        current_diff = pred[k] - truth[l]
                        current_l = l
                        found = True
            if found:
                self.error.append(current_diff)
                t_pred.append(k)
                t_truth.append(current_l)

        self.fp += pred.shape[0] - len(t_pred)
        self.fn += truth.shape[0] - len(t_truth)
        self.tp = len(self.error)