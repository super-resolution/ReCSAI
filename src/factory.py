...
class Factory():
    ...
    def create_point_set(self):
        ...
        distribution = np.random.poisson(1200, n_points)  # maxwell.rvs(size=n_points, loc=12.5, scale=5)
    ...
    def simulate_accurate_flimbi(self, points, on_points):
        """input random distributed convolved and resized image"""
        #todo: simulate world in 0.1 ms

        image = self.create_image()
        ground_truth = self.create_image()
        image, ground_truth, on_points = self.concolve_flimbi_style(image, ground_truth, points, on_points)
    ...
    def flimbi_paint_locs(image, on_points, kernel_array, line, j):
